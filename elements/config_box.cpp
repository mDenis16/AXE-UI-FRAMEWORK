...
namespace ui {
	...
	void config_box::load_config ( std::string hash, bool cloud ) {
		...
		for ( auto it = json_data["data"].begin ( ); it != json_data [ "data" ].end ( ); ++it ) {
			...
			variable_type type = ( variable_type)j [  ( "type" ) ].get<int>();
				...
				case int_type:
				{
					*( int * ) item->pointer = j [  ( "value" ) ].get<int> ( );
					...
					if ( !kb.is_null ( ) && item->key_bind ) {
						...
						if ( !kb [  ( "key" ) ].is_null ( ) )
							 item->key_bind->key = kb [  ( "key" ) ].get<int> ( );

						if ( !kb [  ( "mode" ) ].is_null ( ) )
							item->key_bind->mode = kb [  ( "mode" ) ].get<int> ( );

						if ( !kb [  ( "new_value" ) ].is_null ( ) )
							*( int * ) item->key_bind->new_value = kb [  ( "new_value" ) ].get<int> ( );
					}
				...
				case float_type:
				{
					*( float * ) item->pointer = j [  ( "value" ) ].get<float> ( );
					...
					if ( !kb.is_null ( ) && item->key_bind ) {
						...
						if ( !kb [  ( "new_value" ) ].is_null ( ) )
							*( float * ) item->key_bind->new_value = kb [  ( "new_value" ) ].get<float> ( );
					}
					break;
				}
				case bool_type:
				{
					*( bool * ) item->pointer = j [  ( "value" ) ].get<bool> ( );
					...
					if ( !kb.is_null ( ) && item->key_bind ) {
						...
						if ( !kb [  ( "key" ) ].is_null ( ) )
							 item->key_bind->key = kb [  ( "key" ) ].get<bool> ( );

						if ( !kb [  ( "mode" ) ].is_null ( ) )
							 item->key_bind->mode = kb [  ( "mode" ) ].get<bool> ( );

						if ( !kb [  ( "new_value" ) ].is_null ( ) )
							*( bool * ) item->key_bind->new_value = kb [  ( "new_value" ) ].get<bool> ( );
					}
					break;
				}
				case multibox_type:
				{
					...
					if ( !kb.is_null ( ) && item->key_bind ) {
						...
						if ( !kb [  ( "new_value" ) ].is_null ( ) ) {
							...
							json ja = json::parse ( kb [  ( "new_value" ) ].get<std::string> ( ).c_str ( ) );
	...
	void config_box::save_config ( std::string name, bool cloud, std::string hash ) {
		...
		allJson [  ( "hash" ) ] = hash_name;
		allJson [  ( "cloud" ) ] = cloud;
		allJson [  ( "name" ) ] = name;
		allJson [  ( "author" ) ] = "unknown";
		...
		for ( auto& it : cfg_manager->items ) {
			...
			if ( it->key_bind && it->key_bind->key > 0 ) {
				j [  ( "keybind" ) ][  ( "key" ) ] = it->key_bind->key;
				j [  ( "keybind" ) ][  ( "mode" ) ] = it->key_bind->mode;
			}
			...
			case variable_type::bool_type:
				...
				if ( it->key_bind ) 
					j [  ( "keybind" ) ][  ( "new_value" ) ] = ( bool ) * ( bool * ) it->key_bind->new_value;
			...
			case variable_type::float_type:
				j [  ( "value" ) ] = ( bool ) *( bool * ) it->pointer;
				if ( it->key_bind )
					j [  ( "keybind" ) ][  ( "new_value" ) ] = ( float ) *( float * ) it->key_bind->new_value;
			...
			case variable_type::int_type:
				j [  ( "value" ) ] = ( bool ) *( bool * ) it->pointer;
				if ( it->key_bind )
					j [  ( "keybind" ) ][  ( "new_value" ) ] = ( int ) *( int * ) it->key_bind->new_value;
			...
			case variable_type::multibox_type:
			{
				...
				if ( it->key_bind ) {
					...
					j [  ( "keybind" ) ][  ( "new_value" ) ] = ja_keybind_new_value.dump ( );
				}
